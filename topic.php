...
$sql = "SELECT
		...
		FROM
			topics
		WHERE
			topics.topic_id = " . mysqli_real_escape_string($_SESSION['conn'],$_GET['id']);
...
else
{
	...
	else
	{
		...
		{
			//display post data
			echo '<table class="topic" style="border: 1px">
					<tr>
			...
			$posts_sql = "SELECT
					...
					WHERE
						posts.post_topic = " . mysqli_real_escape_string($_SESSION['conn'],$_GET['id']);
